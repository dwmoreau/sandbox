...
def IntegrateModel(angle, h, f, t, s_norm, kapton_absorption_length, f_range, n=1):
	...
	if n == 1:
		...
		for index, f_here in enumerate(f_int):
			path_length_int[:, :, index] = GetModel(angle, h, f_here, t, s_norm, out='kapton')
...
def GetImage(polarization_fraction=0.9):
	# returns everything...
	...
	# The metadata says the polarization fraction is 0.999. Applying this to the 
	#	detector image seems to suggest that the fraction is more like 0.8.
	#	0.8 seems low, synchrotrons are typically 0.90 - 0.95
	#polarization_fraction = beam.get_polarization_fraction()
	kapton_absorption_length = get_absorption_correction()(wavelength)
	panel = image.get_detector()[0]
	data = image.get_raw_data().as_numpy_array()

	# This creates a mask for the detector
	# pixels == 0 are dead
	# pixels > 5000 are probably bragg peaks
	mask = np.logical_or(
		data == 0,
		data > 5000
		)
	...
	mask[:, :2] = True
	mask[:, -2:] = True
	# This region is a bad spot on the detector with abnormally low pixel counts
	mask[1325:1400, 225:500] = True
	...
	normalized_image = NormalizeImage(data, s, mask, polarization_fraction)
	return data, normalized_image, s, s_norm, mask, kapton_absorption_length
